...
import { StormgateBuildOrderList, StormgateBuildOrders } from "../Components/Main/Stormgate/StormgateBuildOrders";
  ...
  render(
    <MemoryRouter>
      <Providers userApi={mockUserApi} buildOrdersApi={mockBuildOrdersApi} queryClient={queryClient}>
        <Routes>
          <Route path="/" element={<StormgateBuildOrders />} />
          <Route path={AppRoutes.StormgateBuildOrder} element={<RouterTester />} />
        </Routes>
      </Providers>
    </MemoryRouter>
  );
...
describe("Stormgate Build Orders", () => {
  test("Render the component and call the api to fetch data", async () => {
    ...
    await waitFor(() => {
      expect(stormgate).not.toBeNull();
      expect(mockBuildOrdersApi.getStormgateBuildOrders).toHaveBeenCalledTimes(1);
    });
  });
  ...
  test("Render not found page if fetching errors", async () => {
    mockBuildOrdersApi.getStormgateBuildOrders = jest.fn().mockRejectedValue({});
        ...
        <MemoryRouter>
          <Providers userApi={mockUserApi} buildOrdersApi={mockBuildOrdersApi} queryClient={queryClient}>
            <Routes>
              <Route
                path="/"
                element={<StormgateBuildOrderList buildOrders={[]} isFetching={false} hasNextPage={false} fetchNextPage={() => {}} isError={true} />}
              />
            </Routes>
          </Providers>
        </MemoryRouter>
  ...
  test("Render skeleton modal if still fetching", async () => {
    mockBuildOrdersApi.getStormgateBuildOrders = jest.fn(() => new Promise(() => {}));
    await act(async () => {
      render(
        <MemoryRouter>
          <Providers userApi={mockUserApi} buildOrdersApi={mockBuildOrdersApi} queryClient={queryClient}>
            <Routes>
              <Route
                path="/"
                element={<StormgateBuildOrderList buildOrders={[]} isFetching={true} hasNextPage={false} fetchNextPage={() => {}} isError={false} />}
              />
            </Routes>
          </Providers>
        </MemoryRouter>
  ...
  test("Clicking a build order will redirect to '/stormgate/build-order/:id'", async () => {
    mockBuildOrdersApi.getStormgateBuildOrders = jest.fn().mockResolvedValue(wc3BuildOrderMocks);
    const id = "1";
    await act(async () => {
      renderContainer();
    });

    const buildOrderItem = screen.getByTestId("stormgate-build-order-item-" + id);
  ...
  test("Typing in the search filter tile area trigger an api fetch", async () => {
    ...
    await waitFor(() => {
      expect(mockBuildOrdersApi.getStormgateBuildOrders).toHaveBeenLastCalledWith(
...
const locators = {
  buildOrderList: `[data-testid="stormgate-build-order-list"]`,
