...
import { StarcraftBuildOrderList, StarcraftBuildOrders } from "../Components/Main/Starcraft/StarcraftBuildOrders";
  ...
  render(
    <MemoryRouter>
      <Providers userApi={mockUserApi} buildOrdersApi={mockBuildOrdersApi} queryClient={queryClient}>
        <Routes>
          <Route path="/" element={<StarcraftBuildOrders />} />
          <Route path={AppRoutes.StarcraftBuildOrder} element={<RouterTester />} />
        </Routes>
      </Providers>
    </MemoryRouter>
  );
...
describe("Starcraft Build Orders", () => {
  test("Render the component and call the api to fetch data", async () => {
    ...
    await waitFor(() => {
      expect(starcraft).not.toBeNull();
      expect(mockBuildOrdersApi.getStarcraftBuildOrders).toHaveBeenCalledTimes(1);
    });
  });
  ...
  test("Render not found page if fetching errors", async () => {
    mockBuildOrdersApi.getStarcraftBuildOrders = jest.fn().mockRejectedValue({});
  ...
  test("Render skeleton modal if still fetching", async () => {
    mockBuildOrdersApi.getStarcraftBuildOrders = jest.fn(() => new Promise(() => {}));
    await act(async () => {
      render(
        <MemoryRouter>
          <Providers userApi={mockUserApi} buildOrdersApi={mockBuildOrdersApi} queryClient={queryClient}>
  ...
  test("Clicking a build order will redirect to '/starcraft/build-order/:id'", async () => {
    mockBuildOrdersApi.getStarcraftBuildOrders = jest.fn().mockResolvedValue(wc3BuildOrderMocks);
    const id = "1";
    await act(async () => {
      renderContainer();
    });

    const buildOrderItem = screen.getByTestId("starcraft-build-order-item-" + id);
  ...
  test("Typing in the search filter tile area trigger an api fetch", async () => {
    ...
    await waitFor(() => {
      expect(mockBuildOrdersApi.getStarcraftBuildOrders).toHaveBeenLastCalledWith(
...
const locators = {
  buildOrderList: `[data-testid="starcraft-build-order-list"]`,
